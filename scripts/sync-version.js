...
// Read plugin file
const pluginPath = path.join(__dirname, '..', 'we-taxonomy-navigation.php');
...
// Update WE_TAXONOMY_NAV_VERSION constant
pluginContent = pluginContent.replace(
    /define\('WE_TAXONOMY_NAV_VERSION',\s*'[^']*'\);/,
    `define('WE_TAXONOMY_NAV_VERSION', '${version}');`
);
...
`;
        ...
        if (!changelogContent.includes(`[${version}]:`)) {
            const releaseLink = `\n[${version}]: https://github.com/gbyat/we-taxonomy-navigation/releases/tag/v${version}\n`;
